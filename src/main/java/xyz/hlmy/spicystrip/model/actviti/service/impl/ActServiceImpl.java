...
public class ActServiceImpl implements ActService {
    ...
    public byte[] getFlowImgByProcInstId(String procInstId) throws IOException {
        ...
        try {
            ...
            for (HistoricActivityInstance historicActivityInstance : historicActivityInstanceList) {
                ...
                log.debug("已执行的节点[{}-{}-{}-{}]", historicActivityInstance.getId(), historicActivityInstance.getActivityId(), historicActivityInstance.getActivityName(), historicActivityInstance.getAssignee());
            }
            ...
            imageStream = ((CustomProcessDiagramGenerator) processDiagramGenerator).generateDiagramCustom(bpmnModel, "png", highLightedActivitiIdList, runningActivitiIdList, highLightedFlowIds, "宋体", "微软雅黑", "黑体", null, 2.0);
     ...
     *
     */
    HistoricProcessInstance getHistoricProcInst(String procInstId) {
     ...
     *
     */
    List<HistoricActivityInstance> getHistoricActivityInstAsc(String procInstId) {
        return historyService.createHistoricActivityInstanceQuery().processInstanceId(procInstId).orderByHistoricActivityInstanceId().asc().list();
    }
    /**
     ...
     *
     */
    List<Execution> getRunningActivityInst(String procInstId) {
                ...
                if ("parallelGateway".equals(currentActivityInstance.getActivityType()) || "inclusiveGateway".equals(currentActivityInstance.getActivityType())) {
                    ...
                    for (int i = 0; i < historicActivityInstanceList.size(); i++) {
                        ...
                        if (i >= k && historicActivityInstance.getActivityId().equals(currentActivityId)) {
                            log.debug("第[{}]个历史节点和当前节点一致-ActivityId=[{}]", i + 1, historicActivityInstance.getActivityId());
                            ifStartFind = true;
                            // 跳过当前节点继续查找下一个节点
                            continue;
                        }
